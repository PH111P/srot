...
touchscreens = [ i for i in devices if any( ( not b'pen' in i.lower( ) ) and j in i.lower( ) for j in touchscreen_names ) ]
...
g = 8  # (m^2 / s) sensibility, gravity trigger
...
def rotate( state, screen, hooks = [] ):
    s = STATES[state]
    if screen:
        check_call( [ 'xrandr', '--output', screen, '--rotate', s[ 'rot' ] ] )
        needs_map = [ i.decode( 'utf-8' ) for i in devices if len( [ j for j in hooks if j in i.decode( 'utf-8' ) ] ) ]

        ids = [ check_output( [ 'xinput', '--list', '--id-only', i ] ).splitlines( )[ 0 ].decode( ) for i in needs_map ]
        for i in ids:
            check_call( [ 'xinput', '--map-to-output', i, screen ] )
    else:
...
if __name__ == '__main__':
    ...
    current_state = 0
    rotate( current_state, screen, hooks )
